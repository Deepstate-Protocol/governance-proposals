...
| `reentrancy-no-eth` | `DGP001Bootstrap.execute` and `DeepstateMinterController.mint` complete stream bookkeeping after pinned DEEP and Sablier calls | Both entrypoints are `nonReentrant`, record their principal effects before interaction, and rely on transaction atomicity to restore token, stream, accounting, and audit state after any failure. The Bootstrap first burns and records any unsolicited deterministic-address balance, then renounces its temporary DEEP minter role and checks zero execution-time residual balance and allowance before returning. `activateTokenAdministration()` no longer calls Sablier or contains any legacy-endowment logic; it self-grants the Controller's token role, records the current supply baseline, and starts the term under the same reentrancy guard. The immutable DEEP and Sablier endpoints and Sablier's upgrade authority are pinned and rechecked before execution. |
