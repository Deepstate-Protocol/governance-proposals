...
| `reentrancy-no-eth` | `DeepstateMinterController.mint` completes stream bookkeeping after pinned DEEP and Sablier calls | The entrypoint is `nonReentrant` and has no internal issuance-accounting state; transaction atomicity restores both token mints, the allowance, and any attempted stream after a later failure. `activateTokenAdministration()` does not call Sablier or contain any legacy-endowment or issuance-baseline logic; it self-grants the Controller's token role and starts the term under the same reentrancy guard. The immutable DEEP and Sablier endpoints and Sablier's upgrade authority are pinned by construction and the proposal preflight. `DGP001Bootstrap.mint()` is only a Governor check followed by a mint of its fixed amount to the Governor; it has no mutable execution state or Sablier call. DGP-001 grants and immediately revokes its temporary token role, then the Governor approves Sablier and creates the endowment stream directly in the same atomic batch. |
...
intentional revocation through `setOperator(address(0))`, the Factory's decimal-metadata `staticcall` for nonzero pool
tokens (the zero-address native asset uses the fixed 18-decimal scale), source-pinned assembly, interface
naming, abstract proposal functions, and events emitted after calls into pinned live DEEP/Router contracts.
