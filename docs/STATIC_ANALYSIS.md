...
| `reentrancy-no-eth` | `DeepstateMinterController.lockTokenAdministration` stores the 730-day deadline after Sablier returns | The external activation path and every other lifecycle function that can interact with DEEP or Sablier are `nonReentrant`. Deferring the deadline records activation only after the endowment stream succeeds; any callback or later failure reverts the complete role, mint, stream, and deadline transition. DEEP is immutable, while the immutable Sablier endpoint and its upgrade authority are pinned and rechecked before execution. |
