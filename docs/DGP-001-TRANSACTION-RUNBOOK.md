...
This runbook describes the external transaction sequence after the Minter Controller, V1 Controller, and Rewarder
Factory have been deployed and independently verified. The current proposal ID is a pre-deployment candidate and must
be regenerated after the deployment receipts, post-deployment fork block, and final Markdown are pinned.
...
1. **Run the submission gates without broadcasting.** Scan DEEP's complete `RoleGranted` and `RoleRevoked` history and
   ...
   dependency identities, pristine Controller and Factory state, issuance headroom, Sablier configuration, Safe
   configuration, and current absence of any bypass token-level minter. Market idleness is deliberately not required at
   submission.
2. **Submit one proposal transaction.** The intended proposer
   ...
   description)`. The description includes the generated proposer restriction. All eight action values are zero.
...
## The eight calls inside the execution transaction

The Governor makes these calls in this exact order. If any call fails, EVM transaction atomicity rolls back every
earlier call.

1. DEEP grants `DEFAULT_ADMIN_ROLE` to the Minter Controller.
2. The Governor renounces its DEEP `DEFAULT_ADMIN_ROLE`, leaving the Minter Controller as sole token administrator.
3. The Minter Controller calls `lockTokenAdministration()`. It validates the exact idle V1 market, self-grants DEEP's
   token-level minter role, snapshots both legacy `totalAccrued` counters, mints
   `floor((USDG accrued + NVDA accrued) * 30 / 100)` DEEP into a noncancelable and nontransferable one-year linear
   Sablier stream for the Deepstate Inc Safe, and starts the exact 730-day administration term.
4. The Minter Controller grants the Factory only its Controller-local `MINTER_ROLE`.
5. The Router transfers ownership to the Governor-owned V1 Controller.
6. The V1 Controller grants the Factory only its Controller-local `HOOK_MANAGER_ROLE`.
7. The Factory calls `migrateMarket` for NVDA with the `1 NVDA` to `5,000 NVDA` quantity range and both buy sides active.
...
8. The Factory appoints the Deepstate Inc Safe as its revocable operator.

The legacy Rewarder is no longer the Router hook, but it is not destroyed. Its historical registered claims remain
available, while its unused DEEP balance cannot be recovered by this proposal.
